...
for i = 1:numParticles
  ...
  rot1 = normalize_angle(normrnd(u.r1, r1Noise));
  rot2 = normalize_angle(normrnd(u.r2, r2Noise));
  trans = normrnd(u.t, transNoise);
  
  pose = particles(i).pose;
  pose(1) += trans * cos(pose(3) + rot1);
  pose(2) += trans * sin(pose(3) + rot1);
  pose(3) += normalize_angle((rot1 + rot2));
