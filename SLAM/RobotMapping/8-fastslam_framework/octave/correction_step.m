...
for i = 1:numParticles
  ...
  for j = 1:m
    ...
    if (particles(i).landmarks(l).observed == false)
      ...
      particles(i).landmarks(l).sigma = H \ Q / H'; % inv(H) * Q * inv(H)'
      ...
      % TODO: compute the measurement covariance -> (innovation covariance)
      Q_innov = H * particles(i).landmarks(l).sigma * H' + Q;
      
      % TODO: calculate the Kalman gain
      K = particles(i).landmarks(l).sigma * H' / Q_innov;
      
      % TODO: compute the error between the z and expectedZ (remember to normalize the angle)
      innov = [z(j).range; z(j).bearing] - expectedZ;
      innov(2) = normalize_angle(innov(2));
      ...
      particles(i).weight *= mvnpdf(innov', [0, 0], Q_innov);
