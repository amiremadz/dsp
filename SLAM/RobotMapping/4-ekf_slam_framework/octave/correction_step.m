...
for i = 1:m
  ...
  delta = zeros(2, 1);
  delta(1, 1) = mu(2 * landmarkId + 2) - mu(1);
  delta(2, 1) = mu(2 * landmarkId + 3) - mu(2);
  ...
  Hi(1, 1) = -sqrt(q) * delta(1);
  Hi(1, 2) = -sqrt(q) * delta(2);
  Hi(1, 3) = 0;
  Hi(1, 2 * landmarkId + 2) = sqrt(q) * delta(1);
  Hi(1, 2 * landmarkId + 3) = sqrt(q) * delta(2);
    
  Hi(2, 1) = delta(2);
  Hi(2, 2) = -delta(1);
  Hi(2, 3) = -q;
  Hi(2, 2 * landmarkId + 2) = -delta(2);
  Hi(2, 2 * landmarkId + 3) = delta(1);
...
innov = normalize_all_bearings(Z - expectedZ);
